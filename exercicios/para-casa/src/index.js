...
app.listen(7777, () => {
    console.log("O servidor está na porta 7777 :)")
})

app.get("/", (request, response) => {
    response.status(200).json([{
        "message": "deu certoo, está roteando"
    }])
})
...
app.get("/filmes/:id", (request, response) => {
    ...
    let filmeEcontrado = ghibliFilmesJson.find(filme => filme.id == idRequest)
...
app.get("/director", (request, response) => {
    ...
    let filmeEcontrado = ghibliFilmesJson.filter(filme => filme.director.toLocaleLowerCase().includes(directorRequest))
    response.status(200).send(filmeEcontrado)
})
...
app.post("/filmes", (request, response) => {  
    ...
    response.status(201).json([{
        "message": "Seu filmes foi cadastrado com sucesso :)!", 
        novoFilme
    }])
})
